...
3.3.1 |  --- Internet APIs  ---  | |
3.3.1.1 |  YOU CAN ONLY USE REST API ONLY | if the end page has the data in JSON format ..eg https://api.datamuse.com/words?rel_rhy=funny OR https://tastedive.com/api/similar?q=red+hot+chili+peppers%2C+pulp+fiction| 
...
3.3.1.1 |  YOU CAN ONLY USE REST API ONLY | if the end page has the data in JSON format ..eg https://api.datamuse.com/words?rel_rhy=funny OR https://tastedive.com/api/similar?q=red+hot+chili+peppers%2C+pulp+fiction| 
3.3.1.5 | URL Structue for REST APIs - check the end point, it should be a JSON string | 3 parts of the URL (1) the endpoint_URL (2)the question mark: ? (3) the query (key/value pair(s) seperated by a &), for example: https://api.datamuse.com/words?rel_rhy=funny (1) https://api.datamuse.com/words  (2) ? (3) {'rel_rhy': 'sunny', 'max':5} |
3.3.1.5 | -- Calling APIs and getting information -- | >>> `endpoint_URL = 'https://api.datamuse.com/words'` \n  `params_dict = {'rel_rhy': 'sunny', 'max':5}`  \n  `page_response = requests.get(endpoint_URL, params = params_dict)` \n `page_py_object = page_response.json()` |
...
3.3.5 | Cache retrieval: imporving efficiency | since dictionaries are not ordered, and we are passing a dict as a parameter to make an API call, different API calls might be requesting the same page (with params reordered) ...solution ordering the parameters dict ... `params_sorted = {key:params_dict[key] for key in sorted(params_dict)}` ...use of dict comprehension |
...
4.1.3 | class syntax | >> `class Class_name:` ...the convention is to have the Class name start with upper case, e.g. Point, School, Country, and if it is not inhering from a parent class, do no add the parenthesis () | 
