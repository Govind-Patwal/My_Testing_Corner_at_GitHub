...
0.1.3 | some libraries | ### `import turtle` (for turtle graphics), ### `import random` (for floats from 0 to 1 >>> random.random() ... for integers >>> `random.randrange(start, end, step)` ...the only difference is while range() returns all the numbers in the series, randrange() returns only 1 random number  ), ###  `import json` (to convert py_objects to/from JSON strings), ### `import` copy (to deepcopy) , ### `import requests` (to work with APIs) ... not python native ...user after win cmd -> pip3 install requests, ### `import requests_cache` (to cache files), `import webbrowser` (to open webpages) | 
